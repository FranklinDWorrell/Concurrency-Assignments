 ...
 * The <code>Port</code> thread effectively manages a message queue that 
 ...
 */ 
public abstract class Port implements Runnable, IPort {
	int portNo; 
	int networkSize; 
	 ...
	 * the <code>Port</code> and each <code>EndPoint</code>. 
	 * @param the new value for <code>this.doneBroadcasting</code>
	 */ 
	...
	/**
	 * Returns whether or not this <code>Port</code> intends to send
	 ...
	 * the <code>Port</code> and each connected <code>EndPoint</code>.
	...
	/**
	 * Returns a reference to the <code>Port</code>'s message
	 * queue.
	...
	public Message findMessage(int studentId, Class targetType) {
		...
		while (true) {
			...
			if (targetType.isInstance(received)) {
				if (received.getStudentId() == studentId) {
					return received; 
				}
			} 
