...
/**
 * The <code>Port</code> implementation for used with Java sockets. 
 * 
...
public class ChannelPort extends Port {
	...
	public void initialize() {
		...
		System.out.println("All students have arrived and are seated."); 
	} 
		...
		for (int j = 0; j < outs.length; j++) { 
			try {
				...
				System.err.println("Error in socket connection while " + 
								   "broadcasting to Student " + j + "."); 
				e.printStackTrace(); 
			} catch (IOException e) {
				System.err.println("Error broadcasting to Student " + j + "."); 
				e.printStackTrace(); 
			} 
		}
	}
	...
	public void closeAllOutputWriters() {
		for (int j = 0; j < outs.length; j++) {
			try {
				...
				System.err.println("Error closing Student " + j + 
								   "'s ObjectInputStream."); 
			} 
		}
	} 
